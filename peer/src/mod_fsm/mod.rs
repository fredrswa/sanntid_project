...
pub fn run(
    ...
    let mut created_completed_timestamps: Vec<Vec<(i64, i64)>> = vec![vec![(Utc::now().timestamp_millis(), Utc::now().timestamp_millis()); 4]; CONFIG.elevator.num_floors as usize + 1];
        ...
        cbc::select! {
            recv(call_from_io_rx) -> cb_message => {
                if let Ok(call_button) = cb_message {
                    ...
                    println!("{:?}", created_completed_timestamps);
