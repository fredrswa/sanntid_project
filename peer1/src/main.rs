...
/// main function
fn main() -> Result<()> {
    // "cargo run id primary udp_secondary_socket elevator_ip floors peers"
    ...
    let full_state = init_state();

    if !primary {
        // Enters state as listening secondary if not specified as primary
        let listening_socket 
        state = secondary_state(listening_socket)
